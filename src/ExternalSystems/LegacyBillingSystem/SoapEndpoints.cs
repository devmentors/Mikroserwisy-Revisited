...
public static class SoapEndpoints
{
    ...
    private static async Task GetInvoice(HttpContext context)
    {
        try
        {
            ...
            {
                await CreateSoapFault("Client.InvalidRequest", "Missing INV_ID in request")
            ...
            await Results.Content(soapResponse, "text/xml; charset=utf-8")
                .ExecuteAsync(context);
        }
        catch (Exception ex)
        {
            await CreateSoapFault("Server.InternalError", $"Unexpected error: {ex.Message}")
                .ExecuteAsync(context);
        }
    }

    private static async Task GetCustomerInvoices(HttpContext context)
    {
        try
        {
            using var reader = new StreamReader(context.Request.Body);
            ...
            {
                await CreateSoapFault("Client.InvalidRequest", "Missing DOMAIN in request")
                    .ExecuteAsync(context);
                return;
            }
    ...
    private static List<LegacySoapInvoice> GenerateTicketFlowInvoices()
    {
        ...
        return new List<LegacySoapInvoice>
        {
            ...
            new LegacySoapInvoice
            {
                ...
                AMT_CURR = "USD",
                STS_CD = "O",
                DT_ISS = now.AddDays(-65).ToString("yyyyMMdd"),
                DT_DUE = now.AddDays(-35).ToString("yyyyMMdd"),
                DT_PD = "",
                LINE_ITEMS = new LegacyLineItems
                {
                    Items = new List<LegacyLineItem>
