...
<soap:Envelope xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"">
  <soap:Body>
    ...
    private List<LegacySoapInvoiceDto> ParseCustomerInvoicesSoapResponse(string soapXml)
    {
        ...
        return invoicesElement.Elements(ns + "INVOICE")
            .Select(inv => new LegacySoapInvoiceDto(
                INV_ID: inv.Element(ns + "INV_ID")?.Value ?? "",
                CUST_REF: inv.Element(ns + "CUST_REF")?.Value ?? "",
                INV_NUM: inv.Element(ns + "INV_NUM")?.Value ?? "",
                AMT_TOT: inv.Element(ns + "AMT_TOT")?.Value ?? "0",
                AMT_CURR: inv.Element(ns + "AMT_CURR")?.Value ?? "USD",
                STS_CD: inv.Element(ns + "STS_CD")?.Value ?? "",
                DT_ISS: inv.Element(ns + "DT_ISS")?.Value ?? "",
                DT_DUE: inv.Element(ns + "DT_DUE")?.Value ?? "",
                DT_PD: inv.Element(ns + "DT_PD")?.Value ?? "",
